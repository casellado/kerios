...
/** Riquadro informativo NTC 2018 §11.2.5 + Circolare 2019 (sempre visibile, anche
 *  senza prelievi). Solo testo, coerente col riquadro analogo dell'acciaio. */
function RiquadroNorma() {
  return (
    <section className={styles.infoNorma} aria-labelledby="info-norma-titolo">
      ...
      <p>
        ...
        calcestruzzo che — nell’ambito di una stessa ricetta di base — mantiene costanti i requisiti
        di classe di resistenza e classe di esposizione (sono ammesse lievi modifiche dei componenti
        per mantenere le prestazioni). In Kerios i prelievi sono quindi raggruppati per miscela
        omogenea.
      </p>
      <p>La norma prevede due criteri, non convergenti, da usare in modo alternativo:</p>
      <ul>
        <li>
          <strong>Tipo A (§11.2.5.1)</strong> — forfettario: ogni controllo è riferito a un massimo
          di 300 m³ di miscela omogenea ed è costituito da 3 prelievi (terzina). Esito positivo se R
          <sub>cm</sub> ≥ R<sub>ck</sub> + 3,5 e R<sub>c,min</sub> ≥ R<sub>ck</sub> − 3,5.
        </li>
        <li>
      ...
      <p>
        Il tipo di controllo è scelto dall’operatore (Direttore dei Lavori); Kerios lo suggerisce in
        base a volume e numero di prelievi, ma la scelta resta libera.
      </p>
    </section>
  );
}
